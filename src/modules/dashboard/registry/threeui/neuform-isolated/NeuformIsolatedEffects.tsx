...
function transformThinkingButtonSource(source: string, mode: EffectMode) {
  ...
  return source
    ...
    .replace("grd.addColorStop(1, '#272c36');", `grd.addColorStop(1, '${plate[2]}');`)
        .replace(/rgb\(83,92,135\)/g, "rgb(219,234,254)")
        .replace(/rgb\(97,106,150\)/g, "rgb(239,246,255)")
...
function transformGradientCollectionSource(
  ...
  return source
    ...
    .replace(/0, '#ffffff', 'left'/g, `0, '${primaryText}', 'left'`)
...
function transformAudioWordmarkSource(source: string, mode: EffectMode) {
  ...
  return source
    ...
    .replace(/supreme radio/g, "ThreeUI")
    .replace(/#EA3927/g, accent)
    .replace(/#E8EEE9/g, ink)
    .replace(/#E3EDE5/g, ink)
    .replace(/#c9d4cc/g, secondaryInk)
    .replace(/#000/g, background)
