...
function patchDiagnosticsPanel(source: string, { mode }: BakeKnobs) {
  ...
  if (mode === "light") {
    ...
    next = next
      .replace(/ctx\.fillStyle = '#020804';/g, `ctx.fillStyle = '${LIGHT_PAPER}';`)
      .replace(/rgba\(52,211,153,/g, "rgba(4,120,87,");
  }
  return next;
}
...
const EFFECTS = {
  ...
  particleDrift: {
    ...
    patch(source, { size, length, density, mode }) {
      ...
      if (mode === "light") {
        next = next
          .replace(/rgba\(96, 165, 250,/g, "rgba(37, 99, 235,")
          .replace(/rgba\(156, 163, 175,/g, "rgba(36, 48, 68,")
  ...
  defenseLines: {
    ...
    patch(source, { size, length, density, mode }) {
      ...
      if (mode === "light") {
        ...
        next = next
          .replace(/rgba\(220, 38, 38, 0\)/g, "rgba(153, 27, 27, 0)")
