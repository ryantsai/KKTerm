...
// @ts-nocheck -- vendored verbatim from the submerged-snell-ocean reference skill (three-cascade spectral wave simulation); preserved byte-for-byte, matching fujiBackground.tsx's convention for other preserved-algorithm files.
...
interface Cascade {
  ...
  /** Retained only so dispose() can free every GPU texture this cascade owns. */
  spectrum: ReturnType<typeof createSpectrumTexture>
  freqPing: StorageTexture
  freqPong: StorageTexture
}
...
export class WaveSim {
  ...
  /**
   * Frees every GPU texture the cascades own. Not present in the upstream
   * reference (its demo harness never tears a scene down); added so a
   * consumer that mounts/unmounts SubmergedOcean repeatedly — as a
   * dashboard background does whenever the user switches away — doesn't
   * leak a full cascade of storage textures on every dispose.
   */
  dispose(): void {
    for (const cascade of this.cascades) {
      cascade.spectrum.dispose()
      cascade.freqPing.dispose()
      cascade.freqPong.dispose()
      cascade.displacementMaps[0].dispose()
      cascade.displacementMaps[1].dispose()
      cascade.derivativesMap.dispose()
    }
  }
}
