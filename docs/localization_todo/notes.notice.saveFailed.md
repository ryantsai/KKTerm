# `notes.notice.saveFailed`

English: `The note could not be saved. {{error}}`

- **Namespace**: `notes` (Connection notes: the pane-toolbar post-it affordance and the rich-text note editor)
- **UI role**: error
- **User flow**: Status Bar error notice when saving a note fails; {{error}} is the backend message.
