# `notes.confirmDelete.message`

English: `The note for {{name}} and its images are deleted permanently.`

- **Namespace**: `notes` (Connection notes: the pane-toolbar post-it affordance and the rich-text note editor)
- **UI role**: status
- **User flow**: Confirmation body warning that the note and its images go away permanently; {{name}} is the Connection name.
