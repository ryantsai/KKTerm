# `notes.notice.selectUrlForLink`

English: `Select a web address starting with http:// or https:// to link it.`

- **Namespace**: `notes` (Connection notes: the pane-toolbar post-it affordance and the rich-text note editor)
- **UI role**: status
- **User flow**: Status Bar hint telling the user the selection must be an http(s) address to become a hyperlink.
- **Placeholders**: none
