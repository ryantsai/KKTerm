# `notes.notice.loadFailed`

English: `The note could not be opened. {{error}}`

- **Namespace**: `notes` (Connection notes: the pane-toolbar post-it affordance and the rich-text note editor)
- **UI role**: error
- **User flow**: Status Bar error notice when a note cannot be loaded; {{error}} is the backend message.
