# `notes.toolbar.insertDeepLink`

English: `Link to a KKTerm item`

- **Namespace**: `notes` (Connection notes: the pane-toolbar post-it affordance and the rich-text note editor)
- **UI role**: tooltip
- **User flow**: Note editor toolbar: insert a link to another element inside KKTerm (Connection, Workspace, or IT Ops rack device), not a web address.
- **Placeholders**: none
