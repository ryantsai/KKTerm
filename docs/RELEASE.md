...
- App-owned update checks are enabled by default and contact KKTerm release metadata only. This is separate from telemetry: KKTerm does not send analytics, crash reports, terminal contents, Connection data, or secrets as part of update checking. When a newer non-draft, non-prerelease release is available, KKTerm prompts the user; it does not install anything without an explicit click. Microsoft Store and Mac App Store packages are excluded because the platform Store owns their update lifecycle.
- Packaged Windows builds outside Microsoft Store normally check `https://kkterm.ryantsai.com/releases/latest.json`; macOS builds outside Mac App Store and Linux check the signed-only Tauri manifest at `https://kkterm.ryantsai.com/releases/latest-tauri.json`. Both are served by the app-owned Cloudflare release mirror, and GitHub Releases remains the fallback. These requests contain no Connection, Session, Tab, terminal, credential, or analytics data. Platform Store-managed packages make no app-owned update metadata request.
...
- **Install Helper `downloadInstaller` fallbacks.** The provider schema accepts optional `arm64Url` / `arm64FileName`, plus optional `githubRepo` / `githubAssetPattern` fields for versioned vendor installers published on GitHub Releases. On a native ARM64 build (`cfg!(target_arch = "aarch64")`), `Provider::download_target` prefers the ARM64 asset and otherwise falls back to the x64 asset (which still runs under emulation); a GitHub-backed provider resolves the current stable release and matches the architecture-specific asset pattern before downloading. `installer/catalog.v1.json` populates native ARM64 downloads where the vendor URL scheme is deterministic (GitHub CLI, VS Code, rustup, nvm-windows); `winget` providers already auto-select ARM64. Remaining x64-only entries keep working under emulation and can adopt the same two fields once a native URL is confirmed.
...
Store packages use the shared Evergreen WebView2 Runtime included with Windows 11, as recommended by [Microsoft's WebView2 distribution guidance](https://learn.microsoft.com/microsoft-edge/webview2/concepts/distribution). This keeps each architecture package small and lets Microsoft service WebView2 security updates independently. For constrained offline deployments, pass `-WebView2CabUrl` (the Fixed Version `<arch>` package URL) or `-WebView2RuntimePath` (an already-extracted runtime folder) to embed a Fixed Version runtime explicitly. Downloaded Fixed Version runtimes are cached under `%LOCALAPPDATA%\KKTerm\msix-webview2-cache`; `src-tauri/src/main.rs` selects the embedded `WebView2Runtime` folder only when it exists.
...
At runtime, KKTerm recognizes a Store-installed release by the package's Windows `PackageSignatureKind::Store` identity. Those releases do not render the Software Updates subsection in Settings and perform neither startup nor manual app-owned update checks; Microsoft Store handles discovery and installation. A developer- or enterprise-signed sideloaded MSIX is not classified as Store-managed and retains the normal installed-Windows behavior.

## Mac App Store

At runtime, KKTerm recognizes a Mac App Store installation by the standard receipt at `Contents/_MASReceipt/receipt` inside the application bundle. Those releases do not render the Software Updates subsection in Settings and perform neither startup nor manual app-owned update checks; Mac App Store handles discovery and installation. Developer ID-signed and notarized direct-download builds do not contain that receipt and retain the normal signed Tauri updater behavior.
...
It detects the version from the DMG filename and uses the matching `v<version>` GitHub Release when `--tag` is not supplied. It then notarizes and staples the final renamed DMG, writes the checksum, uploads the macOS files with `gh release upload --clobber`, patches the release notes `Direct Downloads` section with the macOS DMG link, and writes `latest.json` with both `darwin-aarch64` and `darwin-x86_64` updater entries pointing at the same universal bundle. After the DMG upload succeeds, it updates `Casks/kkterm.rb` in `ryantsai/homebrew-tap`, validates the cask with Homebrew, commits it, and pushes the tap so users can install with `brew install --cask ryantsai/tap/kkterm`. The `latest.json` `notes` field is copied from the current GitHub Release body so the Tauri updater dialog can show the real release notes. Use `--tag v<version>` to force a specific release, `--skip-build` to upload the latest already-built Tauri DMG/updater bundle, `--skip-notes-patch` to leave the release body unchanged, `--skip-homebrew` for a recovery upload that must not change the tap, and `--dry-run` to print the resolved version, tag, repository, artifact names, and target tap without building or uploading.

The cross-repository Homebrew push uses a write-enabled SSH deploy key scoped to `ryantsai/homebrew-tap`. Store its private half as the `HOMEBREW_TAP_SSH_KEY` secret in the KKTerm repository; the public half belongs in the tap repository's deploy keys with write access. The workflow writes the private key to its temporary runner directory and exposes only that path as `HOMEBREW_TAP_SSH_KEY_PATH`. Local authenticated releases can omit the key and use the current `gh`/Git credentials. Override the destination only when intentionally testing another tap by setting `HOMEBREW_TAP_REPO=<owner/repo>`.
...
The two slices do not link identically, and the x86_64 slice is the fragile one. Its deployment target (`10.13`) predates the OS-provided Swift runtime, so a macOS SDK whose framework stubs re-export a framework's Swift overlay makes the linker record `@rpath/libswift*.dylib` dependencies that the app bundle does not contain - the x86_64 slice of 3000.0.15 linked `@rpath/libswiftCoreMedia.dylib` this way with no `LC_RPATH`, so dyld aborted every Intel launch with "Library not loaded" while Apple Silicon was unaffected. `src-tauri/build.rs` passes `-Wl,-rpath,/usr/lib/swift` on macOS so those system-provided overlays resolve, and `pnpm run package:macos` now fails the package step when a `@rpath/...` dependency of the built app has no matching `LC_RPATH`. Do not remove either guard: the failure is invisible on Apple Silicon and on any machine that only inspects the arm64 slice.
