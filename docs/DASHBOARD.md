...
| --- | --- | --- |
| `builtIn` | TypeScript component in `src/modules/dashboard/widgets/` registered in `builtInRegistry.ts` | Normal React render. Current built-ins are App Launcher, Connection, Notes, AI Coding Usage, PC Info (a Speccy-style system-information snapshot with cache + manual refresh), Network Profiles (saved IPv4/IPv6 configurations with per-adapter nicknames and privileged application), Network Tools (subnet / DNS / speedtest / ping / whois tabs), Generators (QR / cron / password / time / hash tabs), and Converters (unit / currency / image tabs). |
...
- **Dynamic** — `{ kind: "dynamic", dynamic }` referencing one of the local HTML5 animation backgrounds in `src/modules/dashboard/registry/dynamicBackgrounds.tsx` (whitelisted in Rust as `DYNAMIC_BACKGROUND_IDS`). Dynamic backgrounds are app-owned React/canvas/CSS/WebGL/WebGPU animations, not script widgets and not persisted code. They do not react to pointer clicks or presses; Particle Cursor, Silk Aurora, Closing Plasma, and Liquid Chrome may follow unpressed pointer movement. The Componentry-derived WebGL backgrounds live under `src/modules/dashboard/registry/componentry/`; `componentryBackgrounds.tsx` adapts them to KKTerm's animation-visibility lifecycle. The maintained Poseidon v0.0.2 WebGPU ocean sources live under `src/modules/dashboard/registry/poseidon/`; `maelstromNextBackground.tsx` supplies the next v0.1.147 Maelstrom framing and its isolated `poseidon/maelstrom-v0147/` renderer snapshot with the KKTerm visibility lifecycle, while `maelstromBackground.tsx` supplies the five later fixed camera/sky/palette showcase scenes. Poseidon sky textures live under `public/sky/` and each mounted scene owns its texture so concurrent Pane backgrounds cannot change one another.
...
- **Network Profiles** (`src/modules/dashboard/widgets/builtin/network-profiles/`) — a `builtIn` widget for quickly applying a saved IPv4/IPv6 configuration to a user-selected local network adapter. The adapter picker shows the live configuration and accepts a KKTerm-only nickname without renaming the OS interface. The Add action opens a profile form covering name, IPv4/IPv6 mode, address, subnet mask or prefix length, gateway, and DNS, prefilled from the selected adapter; the same form edits a saved profile. Saved profiles are adapter-independent, so one profile can be applied to another selected adapter. A card shows Apply only while its configuration differs from the selected adapter's live settings and otherwise shows a Current marker. Profiles, nicknames, and the last adapter selection use the machine-local SQLite-backed durable UI-state key `kkterm.dashboard.networkProfiles.v1`; they are included in full database backups but intentionally do not travel in portable `.kkwidget` definitions or selective Dashboard export because adapter identities are machine-specific. `network_profiles_snapshot` is read-only and runs its adapter query through a hidden PowerShell process; `network_profiles_apply` accepts a bounded structured request and validates every address before invoking fixed OS tools. Windows uses a short-lived UAC PowerShell child, direct-download macOS uses the system administrator authorization prompt around `networksetup`, and Linux supports NetworkManager through `nmcli` plus polkit. The sandboxed Mac App Store build and Linux systems without NetworkManager remain read-only. Applying a profile always names the selected profile and adapter in a confirmation because network access may be interrupted.
