...
- `dashboard.backgroundModeDynamic` (`dashboard.backgroundDynamicHint`) — app-owned animated backgrounds: `fuji`, `aurora`, `halftone`, `clouds`, `ocean`, `mistySea`, `maelstrom`, `sunGlitter`, `whitecaps`, `waveField`, `openOceanBlue`, `tropicalGreen`, `waters`, `raindrops`, `rainyWindow`, `frostedWindow`, `snow`, `sakura`, `fireflies`, `bubbles`, `aquarium`, `jellyfish`, `lighthouse`, `balloons`, `ricefield`, `lanterns`, `ricefield`, `starfield`, `nebula`, `orbitals`, `embers`, `lava`, `ink`, `dunes`, `savanna`, `matrix`, `topo`, `synthwave`, `circuit`, `crystals`, `cyberpunk`, `taipei101`, `thunderstorm`, `confetti`, `particleCursor`, `heroGeometric`, `ditherPrismHero`, `webglLiquid`, `silkAurora`, `closingPlasma`, `animatedGradient`, `prismGradient`, `liquidChrome`, `predictiveArc`, `liquidForm`, `energyOrb`, `noiseFlow`, `streamConvergence`, `bellField`, `flowField`, `condensation`, `generativeTree`, `ribbonField`, `particleOrb`, `cloudField`, `voidField`, `recursiveErosion`, `quanteraTradingBackground`, `halftoneFlow`, `constellationField`, `particleDrift`, `particleNetwork`, `amberHalftone`, `matrixField`, `gatewayFlow`, `connectivityGraph`, `interfaceLines`, `defenseLines`, `topoField`, `sylvaLivingWorld`, and `templeNight` (keys under `dashboard.dynamicBackgrounds.*`). The five later Poseidon v0.0.2 showcase backgrounds use its WebGPU FFT ocean: a 256×256 spectrum over three cascades (1024 / 144 / 24 m), a camera-centred radial grid reaching the horizon, measured water optics, panorama lighting, aerial perspective, wave-weighted choppiness, and persistent Jacobian whitecaps. `maelstrom` uses the next v0.1.147 Poseidon WebGPU scene with its 250 / 17 / 5 m cascades and fixed above-water camera, while `sunGlitter`, `whitecaps`, and `waveField` use the corresponding README showcase cameras, and `openOceanBlue` and `tropicalGreen` expose its two inherent-optics palettes. The Poseidon showcase cameras stay above the wave field so these backgrounds do not switch into the underwater view. `mistySea` is a portable WebGL recreation of the supplied procedural sea scene: its haze, sun, clouds, water, and palette play a full 24-hour cycle over five minutes in an endless loop, with fixed wave scale `2` and star density `1.0`. `waters` is a three.js ocean derived from the open-source WaterThreeJS scene: a procedural Gerstner wave spectrum over three cascades feeds an analytic atmosphere shared by the sky and the water reflections, with Beer–Lambert depth colour, GGX sun glints, and foam seeded from the wave Jacobian; the camera pans on a fixed sweep, the water column is solved analytically instead of with extra render passes, and its fixed-size ocean mesh concentrates vertices near the camera to prevent visible triangular seams on lower-precision integrated GPUs. The eight Componentry-derived backgrounds are procedural WebGL scenes; hidden/off-screen scenes release their WebGL contexts instead of continuing to render. The newly imported ThreeUI scenes are adapted from the referenced `threeui` project and use the same active-view/document-visibility gating; the excluded CRT, Spark Badge, Hypnotic Loops, Elements, and Portal Field entries remain unregistered while Cloud Field and Flow Field are included. The Dynamic tab uses captured static thumbnail assets so browsing does not mount live renderers; selecting a background applies it to the View. The picker opens centered over the full app window each time, including above any left navigation panel, and its header can be dragged temporarily to reposition it without persisting that location.
...
Dashboard Views, terminal Connection backgrounds, Document viewer backgrounds, and IT Ops drill-view backgrounds use the same shared background picker datasource. Dynamic backgrounds do not react to pointer clicks or presses; iframe-backed ThreeUI renderers relay right-clicks to their owning surface so the normal native shortcut menu remains available. Particle Cursor, Silk Aurora, Closing Plasma, and Liquid Chrome may follow unpressed pointer movement. New preset, media-mode, supported extension, fit, dim, or dynamic background options must be added through the shared picker/registry path so `dashboard.changeBackground`, `terminal.background`, `workspace.fileViewer.background`, and `itops.racks.changeBackground` show the same list.
...
- **Network Profiles** — `dashboard.networkProfilesTitle` / `dashboard.networkProfilesSummary`. Select a local network adapter with `dashboard.networkProfilesAdapter`; use `dashboard.networkProfilesEditNickname` to assign a KKTerm-only nickname without changing the operating-system interface name. `common.add` in the Saved profiles header, or the empty-state `dashboard.networkProfilesEmptyTitle` action with its `dashboard.networkProfilesEmptyHint`, opens `dashboard.networkProfilesAddTitle` — a form with `dashboard.networkProfilesProfileName`, an IPv4 and an IPv6 section (`dashboard.networkProfilesFamilyMode`, `dashboard.networkProfilesAddress`, `dashboard.networkProfilesSubnetMask`, `dashboard.networkProfilesPrefixLength`, `dashboard.networkProfilesGateway`), and `dashboard.networkProfilesDns` with `dashboard.networkProfilesDnsHint`. The form is prefilled from the selected adapter and blocks invalid input with `dashboard.networkProfilesInvalidAddress`, `…InvalidMask`, `…InvalidPrefix`, `…InvalidGateway`, and `…InvalidDns`; IPv4 subnet masks accept a dotted mask or a prefix length. The `dashboard.networkProfilesEditProfile` action reopens the same form for a saved profile. Saved profiles are not bound to their source adapter: choose any adapter, then use `dashboard.networkProfilesApply` on a profile card to apply it to that adapter. A card whose configuration already matches the selected adapter hides Apply and shows `dashboard.networkProfilesCurrent` instead. The confirmation `dashboard.networkProfilesApplyTitle` warns that connectivity may be interrupted. Windows displays UAC, direct-download macOS displays the system administrator prompt, and NetworkManager Linux displays polkit authentication. The Mac App Store build and Linux systems without NetworkManager show a translated read-only capability notice. Profiles and nicknames stay local to this machine and use SQLite-backed durable UI state; they are not embedded in `.kkwidget` files or selective Dashboard exports.

Only one network profile application may run at a time within KKTerm, including
requests from different Widget Instances. A second request is rejected through
`dashboard.networkProfilesApplyError`, rather than queued or interleaved with the
first. Let the current operation finish, refresh the adapter state, then apply
again. Windows snapshots preserve localized adapter names and descriptions; when
...
- **Canvas Toy/Game** — small games, fidget tools, and canvas/WebGL scenes. Use ambient chrome by default; put score/status/controls inside the scene as HUD affordances.
