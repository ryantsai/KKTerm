...
- Custom Modules send the module name, version, publisher, summary, apiVersion, source/trust/enabled/health, the active contribution (title, entrypoint, routing), and the module's granted permissions. It does not send module internals, user data inside the module, or anything the module has not declared permission for; the assistant must not claim the module can do more than its declared capabilities (`ai.customModuleContextLabel`, `ai.customModuleContextIntro`).
...
- Waiting, active tool, and invoked-skill labels use a short word-stagger reveal. Each rotating waiting phrase reveals again when it changes, and its letters repel from the mouse pointer while hovered. Live status labels end with one shared animated ellipsis; any trailing ellipsis supplied by localized status text is removed before the animation is rendered. Completed work returns to the neutral duration summary instead of retaining its live tool or skill indication. The reveal and pointer response become static when the operating system requests reduced motion.
...
- While a response is running, the chat composer card gets the `border-beam` rotate effect (`assistant.composer`): a traveling colorful beam with an inner glow and blurred bloom sweeps the card's rounded border continuously. The beam fades in and out with the run start/finish (600ms), and becomes a static accent ring when the operating system requests reduced motion.
