...
- Waiting, active tool, and invoked-skill labels use a short word-stagger reveal. Each rotating waiting phrase reveals again when it changes, and its letters repel from the mouse pointer while hovered. Completed work returns to the neutral duration summary instead of retaining its live tool or skill indication. The reveal and pointer response become static when the operating system requests reduced motion.
