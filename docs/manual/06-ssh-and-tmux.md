...
Legacy Protocols is not needed merely because an embedded SSH server has a broken dynamic Diffie-Hellman group-exchange implementation. If modern key exchange initialization fails, KKTerm retries once without the dynamic `diffie-hellman-group-exchange-*` methods while retaining modern fixed-group SHA-256/SHA-512 methods. Host-key inspection and the subsequent native SSH Session use the same retry, so a successful trust prompt cannot be followed by the same negotiation failure during Session startup. The retry does not enable SHA-1 by itself; SHA-1 algorithms remain governed by Legacy Protocols.
