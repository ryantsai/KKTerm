...
- Topics: terminal external links, copy/paste, multiline paste confirmation, find in scrollback, font size, Quick Command Bar, quick commands, Quick Command Bundles, save buffer to file, recording terminal output, starting state, quick select, prompt navigation, shell integration, inline images, terminal notifications, color schemes, tutorial targets `terminal.pane`, `terminal.startRecording`, `terminal.openSftp`, `terminal.copySelection`, `terminal.sendToAi`, `terminal.actions`, `terminal.searchBar`, `terminal.surface`
...
The same menu places `terminal.syntaxHighlight` between Color Scheme and Font. `terminal.syntaxHighlightNone` disables local matching and is the default for every new Connection; every built-in and user-owned profile from Settings → Terminal is available below it. The choice is stored on the durable Connection and live-applies without reconnecting. When a profile is enabled, each matching rule's configured font, foreground, background, bold, and italic attributes take precedence over the terminal color scheme, Connection font/color settings, and ANSI styling emitted by Bash or a remote program. Highlighting operates on xterm's parsed visible buffer, never rewrites PTY bytes, never changes copied text, and remains opt-in per Connection.
...
- Paste: `terminal.paste` (default `Ctrl+V`, also `Ctrl+Shift+V`). Multi-line pastes prompt a confirmation `terminal.pasteMultilineConfirm` to prevent accidental command execution. The confirmation stays above adjacent split Panes, including URL Connection browser surfaces, so its actions remain accessible.
...
- For SSH and Telnet, no Connection-lifecycle action is shown while the Session is connected. If startup fails or the live Session ends while its Pane remains open, the first action becomes `connections.reconnect` and starts a new Session in the same Pane. Close the Pane with its dedicated toolbar close control.
