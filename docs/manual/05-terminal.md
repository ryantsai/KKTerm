...
## Saved passwords at sudo and su prompts

When `settings.offerSavedPasswords` is enabled (the default), entering a `sudo` or `su` command in an SSH or Telnet Session and receiving a password prompt opens a compact picker beside that prompt. The current Connection's stored password is first. Other SSH and Telnet Connections with stored passwords follow; `terminal.searchSavedPasswords` filters them by Connection name or the configured hostname/IP address. `terminal.currentConnectionPassword` marks the current Connection, and `terminal.noTerminalPasswords` appears when a search finds no match. The picker never displays password values. Selecting `terminal.sendSavedPassword` reads the chosen secret in the Rust backend and sends the password followed by Enter to that terminal Session. Dismissing the picker or typing into the terminal sends no stored secret. Settings → Terminal can turn off the offer; sending always requires the user's action.

SSH and Telnet Pane headers show the configured IP address as-is or the leftmost label of an FQDN. Hovering the Pane title shows the full host and port in the native tooltip. The saved Connection name and host remain unchanged, and Child Connection Tab titles retain their own names.
...
When an unfocused terminal emits a bell (BEL / Ctrl+G), KKTerm shows `terminal.attentionPending` on its Pane, replaces the normal green Connection Tree status dot with a pulsing green bell, and adds a matching pulsing green bell to its containing Tab. Reduced-motion mode keeps the Tab and Connection Tree bells green and static. Each pending terminal also gets one monochrome bell immediately before Don't Sleep in the Status Bar (`terminal.attention`, tooltip `terminal.attentionFocus`). Clicking that Status Bar bell opens the existing terminal in its Workspace and focuses its input, revealing it if another Pane was maximized. Focusing the terminal by any route clears only its own indicators and Status Bar bell. Multiple terminals have separate bells; repeated rings from one terminal reuse its bell. Closing or ending the Session clears its attention. Bells are automatic, including while KKTerm is in the background, with no preference, sound, flashing, or Status Bar popup. Replacing the Connection Tree dot preserves green as the connectivity cue while the bell shape communicates attention.

For managed SSH tmux Sessions, KKTerm applies `bell-action any` and `visual-bell off` whenever it creates or reattaches the Session. Bells from any window in that tmux Session can reach KKTerm and trigger `terminal.attentionPending` when the terminal is unfocused. These options apply only to the managed tmux Session; KKTerm does not edit the remote `~/.tmux.conf`. Already-connected Sessions pick up the options on their next reconnect.
