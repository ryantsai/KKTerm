...
**Background transfers.** While the SSH-toolbar popup has active or queued transfers, its close action becomes `sftp.minimize`. Clicking it hides the popup and returns keyboard focus to the originating terminal while transfers continue. Escape does the same when the popup itself owns the key; nested dialogs and editable fields keep their own Escape behavior. The terminal's `terminal.openSftp` folder icon pulses with `sftp.backgroundTransferring`; click it to restore the same browser, folders, and transfer history without reconnecting. Reduced-motion mode uses a static activity mark. Each SSH Pane owns its browser, including in split layouts.

An overwrite conflict or other prompt encountered in the background waits until the browser is restored. The folder icon shows a static attention mark and the Status Bar shows `sftp.backgroundAttention`. When the queue finishes, the pulse stops and the Status Bar shows `sftp.backgroundFinished` or `sftp.backgroundFailed`; reopen the browser to review results or cancel remaining transfers. A finished queue can include skipped or canceled files. With no pending transfers, `common.close` ends the browser Session normally. Closing the owning Pane or Tab, or quitting KKTerm, ends its browser Session; minimizing does not make transfers independent of that owner.
...
Each pane header shows the pane label, a **breadcrumb** path, and per-pane actions. Click a breadcrumb segment to jump to that ancestor folder. **Double-click the breadcrumb** (`sftp.editPathTitle`) to switch it to an editable path field: type a local or remote path and press Enter to navigate, or Escape to cancel. Folder-name autocomplete suggests folders from the current listing and inserts the selected folder path with a trailing slash. Outside the Mac App Store build, the local pane's terminal icon (`sftp.openTerminalHereAria`) opens a runtime-only terminal popup in the current local folder using the dedicated Settings → File Explorer terminal preference (`settings.fileExplorerTerminal`); the Store build omits this action. The popup follows the SSH toolbar SFTP popup model: it does not create a workspace Tab or connected Activity Rail item, renders the terminal edge-to-edge without a light dialog gutter, and puts its `terminal.closePane` action at the top-right of the terminal toolbar. Closing it ends the terminal Session. Windows offers normal and administrator variants of Command Prompt, PowerShell, and PowerShell 7; direct-download macOS defaults to zsh; Linux defaults to bash; reusable custom shells come from Terminal settings. In SFTP/FTP browsers this action is local-side only and is not shown on the remote pane. The recent-path icon (`sftp.recentPathsAria`) opens up to the last five visited paths for that pane. The path input accessibility label is `sftp.pathInputAria`.
...
afterward. Tool-initiated uploads and downloads are queued on the same transfer
queue as the UI's own transfers and run one at a time, so those tools return a
transfer id and the transfer's outcome is read back from the transfer status
tool. A queued transfer can always be canceled; a transfer that already started
can be canceled only for SFTP and FTP, because a local File Explorer copy runs
as a single blocking operation. The MCP tools that expose file contents or
mutate the filesystem require Allow-all; the native Assistant uses its normal
Prompt-mode approval.
