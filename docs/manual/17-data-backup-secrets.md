...
When such a bundle is imported, the passphrase decrypts `secrets.enc` and the passwords are re-encrypted into the credential backend on the importing machine. The backend they land in follows the import: importing the Settings group adopts the bundle's secret-store selection (the selection travels inside the Settings segment); otherwise the passwords go to whichever backend the machine already uses. Because a master password never travels in a bundle, when the passwords are destined for the encrypted SQLite store and that store is not already set up and unlocked on this machine, the import dialog asks for `settings.importEncryptedStorePassword` (`settings.importEncryptedStorePasswordHint`). That one master password both sets up the encrypted store on a fresh machine and unlocks an already-configured one; the import then switches the live credential backend to it, persists that selection, and writes the decrypted passwords into it in a single step — so there is no separate "set up encrypted database" step left afterwards and no missing-secret rows.
