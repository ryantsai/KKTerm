...
The same `remoteDesktop.fullscreen.toggle` shortcut enters full screen from the focused RDP/VNC Pane and exits it again. On Windows it is fixed to Ctrl+Alt+Break (stored canonically as `Ctrl+Alt+Pause`) and appears read-only in Shortcuts Settings because the ActiveX control owns that chord while its full-screen HWND has focus. Windows registers the chord on KKTerm's native window using the Control-Break virtual key (`VK_CANCEL`), so a focused VNC canvas receives the toggle. A focused RDP ActiveX control handles the chord itself; KKTerm enables its documented `ContainerHandledFullScreen` mode and responds to the control's enter/leave request events through the same display-size preparation as the hamburger action instead of accepting the control's smaller built-in mode. macOS defaults to Control+Command+F and Linux defaults to F11; those platforms remain configurable. The native registration remains active while ActiveX is full screen and exits that control directly before attempting WebView event routing.
