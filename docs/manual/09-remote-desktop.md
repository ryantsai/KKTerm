...
The same configurable `remoteDesktop.fullscreen.toggle` shortcut enters full screen from the focused RDP/VNC Pane and exits it again. Defaults are Ctrl+Alt+Break on Windows (stored canonically as `Ctrl+Alt+Pause`), Control+Command+F on macOS, and F11 on Linux. The native registration remains active while ActiveX is full screen and exits that control directly before attempting WebView event routing, so a customized shortcut still works when no Tauri window owns keyboard focus.
...
- **Windows RDP** never creates a detached WebView. `enter_rdp_fullscreen` sets the connected `mstscax` control's `FullScreen` property after configuring `ConnectionBarText`, `DisplayConnectionBar`, `PinConnectionBar`, and the restore button. `exit_rdp_fullscreen` sets the property back to false; the windowed ActiveX host has retained its Pane bounds, so no park/reveal or cross-window focus transfer is involved. Pane visibility/bounds pushes query the live `FullScreen` property and pause only while the control reports full screen. Windows `/multimon` still needs pre-connect configuration and is not covered by this toggle.
