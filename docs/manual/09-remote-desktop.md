...
`remoteDesktop.fullscreen.enter` in the RDP/VNC hamburger menu presents the current live Session full screen without reconnecting. VNC and macOS/Linux IronRDP open a separate borderless window that attaches to the Session by id; a VNC attach temporarily makes that window the sole framebuffer consumer and forces a full framebuffer (`refresh_vnc_session`) so it repaints immediately. VNC framebuffers larger than the selected display shrink to fit, while smaller framebuffers stay at their native 1:1 size and remain centered instead of being enlarged. Closing the window returns framebuffer ownership to the Pane and requests another full repaint. Windows RDP asks the connected Microsoft ActiveX control to enter its own native full-screen mode.

The same `remoteDesktop.fullscreen.toggle` shortcut enters full screen from the focused RDP/VNC Pane and exits it again. On Windows it is fixed to Ctrl+Alt+Break (stored canonically as `Ctrl+Alt+Pause`) and appears read-only in Shortcuts Settings because the ActiveX control owns that chord while its full-screen HWND has focus. A focus-gated native hook observes the Control-Break virtual key (`VK_CANCEL`) without consuming it, so a focused VNC canvas receives the local toggle while a focused RDP ActiveX control still receives the chord. KKTerm enables ActiveX's documented `ContainerHandledFullScreen` mode and responds to its enter/leave request events through the same display-size preparation as the hamburger action instead of accepting the control's smaller built-in mode. macOS defaults to Control+Command+F and Linux defaults to F11; those platforms remain configurable.
...
- **Windows RDP** never creates a detached WebView. Connection setup configures `DisplayConnectionBar`, `PinConnectionBar`, the restore button, and `ContainerHandledFullScreen` before `Connect`, then subscribes to `IMsTscAxEvents`. Both `enter_rdp_fullscreen` and ActiveX's `OnRequestGoFullScreen` force `UpdateSessionDisplaySettings` to the current monitor's physical size, enable SmartSizing as a fallback for servers that reject late dynamic updates, set `ConnectionBarText` to the durable Connection name, and set the connected `mstscax` control's `FullScreen` property. `exit_rdp_fullscreen` and `OnRequestLeaveFullScreen` restore the previous Pane display settings and SmartSizing mode before returning to the retained windowed host; no park/reveal or cross-window focus transfer is involved. Pane visibility/bounds pushes query the live `FullScreen` property and pause only while the control reports full screen. Windows `/multimon` still needs pre-connect configuration and is not covered by this toggle.
