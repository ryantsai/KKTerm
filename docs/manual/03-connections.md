...
SSH Add/Edit Connection uses the `connections.auth` tabbed selector for authentication method choices: `connections.keyFile`, `connections.password`, and `connections.sshAgent`. The optional `connections.sshStartupDirectory` field stores a remote directory path for the SSH server; when present, opening the Connection starts the terminal in that path instead of the default remote home directory. Its `connections.browse` action opens `connections.remoteDirectoryPickerTitle`, an SFTP-backed folder picker for choosing that remote path when the Connection can authenticate with saved credentials, a key file, or the SSH agent.
