...
- `connections.closeConnection`, shown while the Connection has one or more open Sessions (including the local live markers held by open File Explorer and Document Tabs). It closes every open Tab and Pane for that Connection. An open SSH, Telnet, or Serial Pane also shows `connections.reconnect`, whether its Session is connected or disconnected, and restarts the Session in the existing Pane. An idle Connection that has not been opened shows neither action.
...
The Connection Tree provides three fixed shortcuts. They act on the focused Connection row and fall back to the currently selected Connection when the blank list surface has focus. F5 additionally works from any non-editable control in the Connection Tree pane:
...
- **F5** reconnects every open SSH, Telnet, or Serial terminal Pane for that Connection. If the Connection has no open terminal Pane, F5 opens it through the normal Connection path instead. The tree owns this key only while it has focus, so F5 in xterm, RDP, VNC, SFTP, URL, and other Session surfaces continues to reach that surface unchanged.
...
The handler is bound to the Connection Tree pane, and the blank scrolling list surface is keyboard-focusable, so F5 can use the selected Connection without requiring a row itself to retain focus. Keyboard focus does not add a browser outline around the Tree or its item buttons; the selected row keeps its normal background highlight. The tree search field and active rename inputs remain excluded; F2 and Delete also remain scoped to the scrolling Tree rather than header actions. F5 or Delete in a terminal or remote-session Pane is never intercepted, and folders and Child Connection Tabs keep their existing context-menu rename/delete. With the default single-click-opens-Connection behavior, clicking a Connection opens it and moves focus into the opened Pane; an accidental double-click still opens it only once. Click the blank Tree list surface or Tab back into the Connection Tree pane before using these keys. F2, F5, and Delete are fixed tree commands and are not part of the customizable Settings → Shortcuts list.
...
Expand `connections.rdpAdvancedOptions` to see the administrative-session, redirection, and bitmap-cache toggles. Each option occupies a full-width row with its switch aligned on the right, leaving room for localized labels, including zh-TW, without character-by-character wrapping.
