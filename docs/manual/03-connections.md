...
- `connections.closeConnection`, shown while the Connection has one or more open Sessions (including the local live markers held by open File Explorer and Document Tabs). It closes every open Tab and Pane for that Connection. If an open SSH or Telnet Pane has no live Session because startup failed or the remote side disconnected, the same menu position changes to `connections.reconnect` and restarts the Session in the existing Pane. An idle Connection that has not been opened shows neither action.
