...
- Title bar: `app.titlebar.appName`, `app.titlebar.appNameVersion`; `src/app/TitleBar.tsx`, `src/app/app.css`; "app version", "hover app name", "hover title-bar icon", "green blue color shift", "reduced motion".
...
The custom title bar is always enabled. Its compact label (`app.titlebar.appName`) shows the app name. Hovering the app icon or name reveals `app.titlebar.appNameVersion`, including the installed version, with one staggered green-to-blue color shift before settling into the title bar's normal text color. Moving away hides the version again; hovering other title-bar controls or empty space does not reveal it. Reduced-motion preferences show the full title immediately without animation. Assistive technology can read the full title at all times, and the icon and text remain window drag regions.
