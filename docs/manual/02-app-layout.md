...
1. **Workspace switcher** (`workspace.workspaceSwitcher`) — the Default Workspace, user-created Workspaces, and `workspace.newWorkspace`. The New Workspace dialog uses `workspace.workspaceDetails` for the required name, icon, and `workspace.workspaceIconColor` setup; its icon picker search placeholder is `common.searchForMore` because the initial grid shows only a bounded subset of the bundled icon catalog. The `connections.iconForeground` palette is shown inside the icon picker only for recolorable Reicon/Lucide fallback glyphs and the letter-avatar fallback, while the `connections.iconBackground` palette stays beside the picker; together they customize icon strokes and the icon chip. Clearing the foreground restores automatic light/dark contrast against the selected background, including the visibly bounded white choice. Its footer follows the host platform: macOS places Cancel before the outer-right primary action, while Windows and Linux place the primary action before Cancel. While `workspace.workspaceDetails` is being entered with an IME, candidate-confirming or candidate-canceling Enter/Escape keystrokes stay with the IME; they do not activate `common.save` or dismiss the dialog. If existing Workspaces contain Connections, `workspace.importConnections` can copy selected Connections from the source chosen by `workspace.importFromWorkspace`; the import list can be narrowed with `workspace.searchConnections`, the `workspace.filterConnectionTypes` dropdown, `workspace.selectAllConnections`, and `workspace.deselectAllConnections`. The AI Assistant and built-in MCP server expose the same Workspace list/create/rename/reorder/delete storage path; out-of-band mutations reload the Activity Rail through `workspaces-changed`, and deletion remains approval-gated because it removes that Workspace's durable Connections and folders.
2. **Module destinations** — Dashboard, IT Ops, Install Helper, and Screenshots (`screenshots.railLabel`, tutorial target `app.activityRailScreenshots`) appear when enabled. Enabled Custom Module contributions occupy their own divider group immediately after all built-in Modules. Drag a Custom Module icon to reorder it within that group; the icon cannot cross either divider into the built-in Module or Connection sections, and the order persists locally across launches. When no Custom Modules are enabled, the group and its leading divider are omitted so the rail keeps its previous layout. Curated and local Custom Module packages use declared SVG artwork as a monochrome, theme-aware rail glyph only after the host accepts its inert content and 64 KiB size bound; missing or rejected artwork uses the generic Package glyph. After a Custom Module has started, visiting another Module hides and retains the selected contribution so returning does not reload its web app or discard its live in-memory state. Selecting a different Custom Module contribution replaces that retained runtime.
3. **Connection Rail** (`app.connectionRail`) — a divider group `app.connectedConnectionsRail` that follows the Custom Module group when present and shows:
...
4. **Don't Sleep** (`app.activityRailDontSleep`) — the keep-awake control. The tooltip changes between `app.dontSleepEnabledTooltip` and `app.dontSleepDisabledTooltip`; toggling shows the shared status popup above the Status Bar. Right-clicking opens a native schedule menu with `app.shutdownTimerAfter` choices for 15 minutes, 30 minutes, 1, 2, 4, 8, or 24 hours. While a schedule is active, the same menu also offers `app.shutdownTimerCancel`. The Mac App Store build offers no schedule menu at all: macOS shutdown needs a System Events Apple event that the sandboxed Store build has no automation entitlement to send, so the timer could never fire. Every other build keeps the menu.
5. **Settings** — anchored to the bottom of the rail.

The whole rail uses `app.primaryNav` as its accessible label. It is non-selectable app chrome: dragging across rail icons never highlights or drags their artwork. Module identity tiles, titles, and compact metadata in the shared Module header are likewise non-selectable, while editable header controls retain normal text selection. Tooltips come from `RailTooltip` (delayed hover/focus). In the Windows Tauri runtime, the same helper uses a native topmost tooltip so rail labels can appear above RDP ActiveX surfaces. Native browser `title` tooltips are forbidden here.
...
On Windows, hiding the Tab Strip also moves the `app.openFile` entry point to the compact chevron beside the KKTerm title. It remains Workspace-only and opens the native file picker directly, like the Tab Strip plus.
...
Owned by `src/modules/workspace/StatusBar.tsx`. Its roles are:
...
2. **Transient notifications and progress** — `showStatusBarNotice` renders Pulse-style popups above the Status Bar, with direct tone icons for success, info, warning, and error. Every transient informational outcome, success confirmation, warning, and error from a Module, page, dialog, or background operation routes through this popup surface instead of inline result text, one-off toast surfaces, banners, snackbars, or local status systems. Long popup messages can wrap to three lines before truncation. Success messages default to 2 seconds; info and warning messages default to 5 seconds; errors stay until dismissed. These auto-dismiss popups do not show a countdown bar. Routine determinate work uses the compact `showStatusBarInlineProgress` variant on the right inside the Status Bar; its translated task label, animated bar, and percentage stay visible without covering app content. File Viewer loading and IT Ops IPAM scans use this inline form. High-salience or cancelable work uses the prominent portaled `showStatusBarProgress` variant instead. App update downloads retain that prominent variant; Windows downloads can be cancelled with `settings.updateDownloadCancel`, while completed downloads remain at 100% for three seconds before installation.
3. **Watchdog state** — when one or more watchdogs exist, the right side shows an animated watchdog icon with `watchdog.statusBarLabel`. Activating it opens a list of running or terminal-undismissed watchdogs. Selecting a watchdog opens a centered app-owned dialog with state, elapsed time, watch summary, next check, exit condition, notification method, action mode, recent values, trigger events, AI interventions, and report actions. Its footer keeps Save Report as an auxiliary action, separates stopping a running Watchdog from closing the dialog, and follows the host platform's action order.
...
The custom title bar is always enabled. Its compact label (`app.titlebar.appName`) shows the app name. Hovering the app icon or name reveals `app.titlebar.appNameVersion`, including the installed version, with one staggered green-to-blue color shift before settling into the title bar's normal text color. Both states use the current App UI font and preserve its normal letter spacing and kerning during the animation. Moving away hides the version again; hovering other title-bar controls or empty space does not reveal it. Reduced-motion preferences show the full title immediately without animation. Assistive technology can read the full title at all times, and the icon and text remain window drag regions.

Panel-toggle icons appear immediately before the window minimize/maximize/close controls. The Connections panel icon (`app.connections`) appears only while Workspace is active. The AI Assistant robot icon (`app.aiAssistant`) appears in every Module and page; when the panel is collapsed, the robot icon becomes muted instead of showing a right-edge collapsed strip.
