...
2. **Module destinations** — Dashboard, IT Ops, Install Helper, and Screenshots (`screenshots.railLabel`, tutorial target `app.activityRailScreenshots`) appear when enabled. Enabled Custom Module contributions follow the built-ins. KKTerm-curated packages use their declared SVG artwork as a monochrome, theme-aware rail glyph; local packages use the generic Package glyph.
