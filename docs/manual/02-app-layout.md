...
2. **Module destinations** — Dashboard, IT Ops, Install Helper, and Screenshots (`screenshots.railLabel`, tutorial target `app.activityRailScreenshots`) appear when enabled. Enabled Custom Module contributions follow the built-ins. Curated and local Custom Module packages use declared SVG artwork as a monochrome, theme-aware rail glyph only after the host accepts its inert content and 64 KiB size bound; missing or rejected artwork uses the generic Package glyph.
