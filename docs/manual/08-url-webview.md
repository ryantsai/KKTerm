...
The right-click `dashboard.qrSaveImage` command opens the native Save As dialog immediately, then captures the complete page to the selected PNG file. The URL toolbar screenshot menu's `webview.capturePageToClipboard` action uses the same full-page image generator but writes the PNG to the OS clipboard. Full-page capture reads the document and viewport dimensions, scrolls through every horizontal and vertical tile, captures the real native WebView surface at each position, stitches the tiles at device-pixel resolution, and restores the original scroll position even when capture fails. Region capture remains a visible-area selection. `workspace.sendEntirePanelToAi` captures only the currently visible URL Pane and does not use full-page stitching. The stitcher rejects pages beyond the runtime's safe canvas dimensions instead of silently cropping them.
...
- Screenshot menu: region capture copies a visible selection; `webview.capturePageToClipboard` captures and stitches the page's complete scrollable width and height.
