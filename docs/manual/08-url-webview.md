...
- Reload: `webview.reload`. F5 also reloads the current page when the embedded URL surface has keyboard focus. In the main app shell, the browser reload itself is suppressed because reloading the shell can detach live Session chrome from its native URL surface, but the key still reaches the focused KKTerm surface: the Connection Tree uses F5 to reconnect/open its selected Connection, and terminal Panes may pass it to their shell or TUI.
