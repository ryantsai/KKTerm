...
directly. The host reads the export bytes with a local browser `fetch`, so the
Module CSP permits `connect-src` for `'self' blob: data:` only; remote targets
still require a declared `networkFetch` grant. The native WebView downloader is always denied, and cross-origin download links are
