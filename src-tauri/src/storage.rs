...
-- VLANs: durable global records, global to the Module like itops_tasks and
-- IPAM. Deliberately a table rather than a field inside a Network Map's
-- graph_json, because VLAN 30 drawn on two maps has to be the same VLAN;
-- map-local VLANs would make that a coincidence of spelling and would need a
...
const AI_PROVIDER_CREDENTIALS: &[(&str, &str, &str)] = &[
    ...
    ("zai", "Z.ai", "Z.ai API key"),
    ("moonshot", "Moonshot AI", "Moonshot AI API key"),
...
fn validate_ai_provider_settings(
    ...
    settings.provider_kind = match settings.provider_kind.trim().to_lowercase().as_str() {
        ...
        "zai" | "z-ai" | "z_ai" | "z.ai" => "zai".to_string(),
        "moonshot" | "moonshot-ai" | "moonshot_ai" | "moonshot ai" => "moonshot".to_string(),
