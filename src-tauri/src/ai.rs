...
const TUTORIAL_TOOL_KNOWN_TARGETS: &str = concat!(
    ...
    "terminal.pane, terminal.attention, terminal.tmuxSessions, terminal.sshPortRedirect, terminal.startRecording, terminal.openSftp, notes.openNote, terminal.copySelection, terminal.sendToAi, terminal.actions, terminal.searchBar, terminal.surface with navigation page=workspace; ",
...
fn ai_tool_definitions_with_skills(
    ...
    if settings.sessions() {
        ...
        tools.push(tool_definition(
            ...
            "Queue an upload of a local path into the current SFTP/FTP directory, or a copy into a local File Explorer directory. Transfers run one at a time on the browser's transfer queue, so this returns the queued transferId immediately; poll session_file_browser_transfer_status for progress and the final state. Set overwriteBehavior to overwrite to replace an existing destination.",
        ...
        tools.push(tool_definition(
            ...
            "Queue a download of a remote path into a local directory, or a copy into a destination directory in local File Explorer. Transfers run one at a time on the browser's transfer queue, so this returns the queued transferId immediately; poll session_file_browser_transfer_status for progress and the final state. Set overwriteBehavior to overwrite to replace an existing destination.",
        ...
        tools.push(tool_definition(
            "session_file_browser_transfer_status",
            "Read the full file browser transfer queue, including progress, completion, failure, and cancellation states. session_state only reports counts, so use this after queueing an upload or download.",
            json!({"type":"object","properties":{"tabId":{"type":["string","null"]}},"required":[]}),
        ));
        tools.push(tool_definition(
            "session_file_browser_cancel_transfer",
            "Cancel a file browser transfer by transferId. A queued transfer is canceled immediately; a transfer that already started is canceled only when the transport supports it (local File Explorer copies cannot be).",
...
fn system_cleaner_tool_definitions() -> Vec<OpenAiToolDefinition> {
    ...
    vec![
        ...
        tool_definition(
            "system_cleaner_scan",
            "Explicitly scan one Windows drive and return storage, cleanup, recommendation, and installed-application analysis using KKTerm's built-in Rust scanner.",
